...
function getErrorMessage(error) {
  ...
  if (error.response.status === 409) return error.response.data?.detail || 'Não foi possível eliminar esta leccionação.';
...
function LecionacaoFormModal({ mode, form, professores, disciplinas, turmas, onChange, onClose, onSubmit, isSubmitting }) {
  const title = mode === 'edit' ? 'Editar Leccionação' : 'Nova Leccionação';
        ...
        <div className="modal-dialog modal-lg modal-dialog-centered">
          <form className="modal-content sigep-modal" onSubmit={onSubmit}>
            <div className="modal-header">
              <div>
                <p className="eyebrow mb-1">Leccionações</p>
...
function LecionacoesPage() {
  ...
  async function handleSubmit(event) {
    ...
    try {
      if (modalMode === 'edit' && selected) {
        ...
        setSuccess('Leccionação atualizada com sucesso.');
      } else {
        await createLecionacao(buildPayload(form));
        setSuccess('Leccionação criada com sucesso.');
      }
      closeModal();
  ...
  async function handleDelete(lecionacao) {
    const confirmed = window.confirm(`Eliminar a leccionação de "${lecionacao.professor_info?.nome}"?`);
    if (!confirmed) return;
    setError('');
    setSuccess('');
    try {
      await deleteLecionacao(lecionacao.id);
      setSuccess('Leccionação eliminada com sucesso.');
      await loadLecionacoes(page);
    } catch (requestError) {
      setError(getErrorMessage(requestError));
    }
  ...
  return (
    <div className="page-stack">
      <PageHeader
        title="Leccionações"
        breadcrumbs={['Leccionações']}
        actions={<button className="btn btn-primary" type="button" onClick={openCreateModal}><i className="bi bi-plus-lg" />Nova Leccionação</button>}
      />

      {error && <div className="alert alert-danger">{error}</div>}
      {success && <div className="alert alert-success">{success}</div>}

      <section className="sigep-toolbar" aria-label="Filtros de leccionações">
      ...
      <section className="table-card">
        <div className="table-responsive">
          <table className="table sigep-table align-middle">
            ...
            <tbody>
              {isLoading && <tr><td colSpan="7" className="text-center text-muted py-4">A carregar leccionações...</td></tr>}
              {!isLoading && lecionacoes.length === 0 && <tr><td colSpan="7" className="text-center text-muted py-4">Nenhuma leccionação encontrada.</td></tr>}
      ...
      <nav aria-label="Paginação de leccionações" className="sigep-pagination">
